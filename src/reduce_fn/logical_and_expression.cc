...
env_t logical_and_expression_direct(deque<env_t> &env)
{
	return env[0];
}
reg_sym(logical_and_expression_direct);

env_t logical_and_expression_and(deque<env_t> &env)
{
	if (env[1].any->get<tok_t>().type != tok_land) {
		err_msg("Expected &&");
	}
	...
	var_t ret_var = emit_and(var1, var2);
